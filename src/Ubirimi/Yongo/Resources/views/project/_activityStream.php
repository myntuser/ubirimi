        ...
        <div>
            ...
            <table>
                <?php foreach ($data as $userId => $historyData): ?>
                    <tr>
                        <td valign="top">
                            <img width="65px" style="margin-right: 4px"
                                 src="<?php echo UbirimiContainer::get()['repository']->get(
                                     UbirimiUser::class
                                 )->getUserAvatarPicture(
                                     array('avatar_picture' => $userData[$userId]['picture'], 'id' => $userId),
                                     'big'
                                 ) ?>"/>
                        </td>
                        <td>
                            <?php foreach ($historyData as $dateEvent => $eventDatas): ?>
                                ...
                                <?php if ($eventDatas[0]['event'] == 'event_history'): ?>
                                    <?php echo LinkHelper::getUserProfileLink(
                                        ...
                                        $userData[$userId]['last_name']
                                    ) ?>
                                    updated <a
                                        href="/yongo/issue/<?php echo $eventDatas[0]['issue_id'] ?>"><?php echo $eventDatas[0]['code'] . '-' . $eventDatas[0]['nr'] ?></a>
                                    <ul>
                                        <?php foreach ($eventDatas as $event): ?>
                                            <li>
                                                ...
                                                <?php else: ?>
                                                    updated the <?php if (isset(Field::$fieldTranslation[$event['field']])) {
                                                        echo Field::$fieldTranslation[$event['field']];
                                                    } else echo $event['field'] ?>
                                ...
                                <div><?php echo Util::getFormattedDate($dateEvent, $clientSettings['timezone']) ?>&nbsp;
                                    <a href="#"
                                       id="add_project_history_comment_<?php echo $eventDatas[0]['issue_id'] . '_' . $index ?>">Comment</a>
                                </div>
                                <div
                                    id="project_history_comment_<?php echo $eventDatas[0]['issue_id'] ?>_<?php echo $index ?>"></div>
