...
<div class="headerPageBackground">
    <table width="100%">
        <tr>
            ...
            <td>
                <div class="headerPageText"><?php echo $project['name'] ?> > <a
                        href="/yongo/project/reports/<?php echo $projectId ?>">Reports</a> > Work Done Distribution
                </div>
            </td>
        </tr>
    </table>
...
<div class="pageContent">
    ...
    <?php if ($hasAdministerProject): ?>
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <td>
                    <a id="btnNew" href="/yongo/administration/project/<?php echo $projectId ?>"
                       class="btn ubirimi-btn">Administer Project</a>
                </td>
            </tr>
        </table>
    <?php else: ?>
    ...
    <div>
        <form name="from_work_distribution" method="post"
              action="/yongo/project/reports/<?php echo $projectId ?>/work-done-distribution/<?php echo $dateFrom ?>/<?php echo $dateTo ?>">
            <span>Interval</span>
            <span>From</span>
            <input type="text"
    ...
    <table class="table table-hover table-condensed">
        ...
        <?php foreach ($workDataPrepared as $assignee => $work): ?>
            <tr>
                ...
                <?php foreach ($issueTypes as $issueType): ?>
                    <td><?php if (isset($work[$issueType['name']])) {
                            echo $work[$issueType['name']];
                        } else {
                            echo '0';
                        } ?></td>
                <?php endforeach ?>
