...
<div class="pageContent">
    ...
    <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
        <tr>
            <td><a id="btnNewFilter" href="/yongo/issue/search" class="btn ubirimi-btn"><i class="icon-plus"></i> Create
                    Filter</a></td>
            <td><a id="btnDeleteFilter" href="#" class="btn ubirimi-btn disabled"><i class="icon-remove"></i> Delete</a>
            </td>
        </tr>
    </table>
        ...
        <table class="table table-hover table-condensed">
            ...
            <?php while ($filter = $filters->fetch_array(MYSQLI_ASSOC)): ?>
                <tr id="table_row_<?php echo $filter['id'] ?>">
                    <?php $boards = UbirimiContainer::get()['repository']->get(Board::class)->getByFilterId(
                        $filter['id']
                    ) ?>
                    ...
                    <td>
                        <?php
                        $isFavourite = UbirimiContainer::get()['repository']->get(
                            IssueFilter::class
                        )->checkFilterIsFavouriteForUserId($filter['id'], $loggedInUserId);
                        $subscriptions = UbirimiContainer::get()['repository']->get(
                            IssueFilter::class
                        )->getSubscriptions($filter['id']);
                        if ($isFavourite) {
                    ...
                    <td>
                        <?php if ($subscriptions): ?>
                            <a href="/yongo/filter/<?php echo $filter['id'] ?>/subscription"><?php echo $subscriptions->num_rows ?>
                                subscriptions</a>
                        <?php else: ?>
