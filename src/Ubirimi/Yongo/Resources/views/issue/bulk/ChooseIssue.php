...
<div class="pageContent">
    ...
    <ul class="nav nav-tabs" style="padding: 0px;">
        <li class="active"><a
                href="/yongo/issue/bulk-choose?<?php echo $session->get('bulk_change_choose_issue_query_url') ?>">Choose
                Issues</a></li>
    ...
    <form action="/yongo/issue/bulk-choose?<?php echo $session->get('bulk_change_choose_issue_query_url') ?>"
          method="post" name="bulk_operation">
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <td>
                    <button type="submit" class="btn ubirimi-btn" style="margin-left: 0px" name="next_step_2">Next
                        Step
                    </button>
                    <a href="/yongo/issue/search?<?php echo $session->get('bulk_change_choose_issue_query_url') ?>"
                       class="btn ubirimi-btn">Cancel</a>
                </td>
            </tr>
        </table>
        ...
        <?php if (isset($issuesCount) && $issuesCount > 0): ?>
            ...
            while ($issue = $issues->fetch_array(MYSQLI_ASSOC)) {
                ...
                $htmlOutputIssueRow .= '<td class="tdElementList issueDC">' . date(
                        'j/M/Y',
                        strtotime($issue['date_created'])
                    ) . '</td>';
                ...
                $htmlOutputIssueRow .= '</td>';
                $htmlOutputIssueRow .= '<td class="tdElementList issueUR">' . LinkHelper::getUserProfileLink(
                        $issue[Field::FIELD_REPORTER_CODE],
                        SystemProduct::SYS_PRODUCT_YONGO,
                        $issue['ur_first_name'],
                        $issue['ur_last_name']
                    ) . '</td>';
                $htmlOutputIssueRow .= '<td class="tdElementList issueUA">' . LinkHelper::getUserProfileLink(
            ...
            if ($htmlOutputIssueRow != '') {
                ...
                $htmlOutputIssueRow .= '<tr id="table_row_' . $issue['id'] . '">';
                $htmlOutputIssueRow .= '<td colspan="' . count(
                        $columns
