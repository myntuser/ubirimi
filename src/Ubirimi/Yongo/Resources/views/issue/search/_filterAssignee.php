    ...
    <tr>
        <td>
            <select id="search_assignee" name="search_assignee[]" multiple="multiple" class="search_combobox">
                <option <?php if (!isset($getSearchParameters['assignee']) || in_array(
                        -1,
                        $getSearchParameters['assignee']
                    )
                ...
                <?php for ($i = 0; $i < count($searchCriteria['all_client_user_assignee']); $i++): ?>
                    ...
                    <option <?php if ($found) echo 'selected="selected"' ?>
                        value="<?php echo $searchCriteria['all_client_user_assignee'][$i]['id'] ?>"><?php echo $searchCriteria['all_client_user_assignee'][$i]['first_name'] . ' ' . $searchCriteria['all_client_user_assignee'][$i]['last_name']; ?></option>
                <?php endfor ?>
            </select>
        </td>
    </tr>
