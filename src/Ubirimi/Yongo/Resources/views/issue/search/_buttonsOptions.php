...
?>
    <div class="btn-group" style="float: right; margin-left: 5px;">
        <?php if (Util::checkUserIsLoggedIn() && $projectsForBrowsing): ?>
            <a href="#"
               class="btn ubirimi-btn dropdown-toggle <?php if (!$hasGlobalBulkPermission): ?>disabled<?php endif ?>"
    ...
    <div class="btn-group" style="float: right; margin-right: 0px;">
        <a href="#" class="btn ubirimi-btn dropdown-toggle <?php if (!($projectsForBrowsing && $query)) {
            echo 'disabled';
        ...
        <?php if ($projectsForBrowsing && $query): ?>
            <ul class="dropdown-menu pull-right">
                ...
                <li><a href="/yongo/issue/printable-list-full-content?<?php echo urldecode($query); ?>">Full Content</a>
                </li>
            </ul>
        <?php endif ?>
    </div>
