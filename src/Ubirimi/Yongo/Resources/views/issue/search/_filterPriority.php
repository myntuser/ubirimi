<table cellpadding="0" cellspacing="0" border="0">
    ...
    <tr>
        <td>
            <select id="search_issue_priority" name="search_issue_priority[]" multiple="multiple"
                    class="search_combobox">
                <option <?php if (!isset($getSearchParameters['priority']) || in_array(
                        -1,
                        $getSearchParameters['priority']
                    )
                ...
                <?php for ($i = 0; $i < count($searchCriteria['all_client_issue_priority']); $i++): ?>
                    ...
                    <option <?php if ($found) echo 'selected="selected"' ?>
                        value="<?php echo $searchCriteria['all_client_issue_priority'][$i]['id'] ?>"><?php echo $searchCriteria['all_client_issue_priority'][$i]['name']; ?></option>
                <?php endfor ?>
            </select>
        </td>
    </tr>
