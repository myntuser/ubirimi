    ...
    <td>
        <span>
            <select id="search_issue_type" name="search_issue_type[]" multiple="multiple" class="search_combobox">
                <option <?php if (!isset($getSearchParameters['type']) || in_array(
                        -1,
                ...
                <?php for ($i = 0; $i < count($searchCriteria['all_client_issue_type']); $i++): ?>
                    ...
                    <option <?php if ($found) echo 'selected="selected"' ?>
                        value="<?php echo $searchCriteria['all_client_issue_type'][$i]['id'] ?>"><?php echo $searchCriteria['all_client_issue_type'][$i]['name']; ?></option>
                <?php endfor ?>
            </select>
        </span>
    </td>
