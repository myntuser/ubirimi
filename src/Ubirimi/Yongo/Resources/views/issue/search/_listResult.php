    ...
    <?php if (isset($issuesCount) && $issuesCount > 0): ?>
        ...
        <table class="table table-hover table-condensed">
            ...
            <?php while ($issue = $issues->fetch_array(MYSQLI_ASSOC)): ?>
                ...
                $slaData = UbirimiContainer::get()['repository']->get(Issue::class)->updateSLAValue(
                    $issue,
                    $clientId,
                    $clientSettings
                );
                ...
                <tr>
                    <?php for ($i = 0; $i < count($columns); $i++): ?>
                        ...
                        <?php if ($columns[$i] == 'type'): ?>
                            <td class="issueType">
                                <img
                                    title="<?php echo $issue['type_name'] . ' - ' . $issue['issue_type_description'] ?>"
                                    height="16px"
                                ...
                                <?php for ($j = 0; $j < count($slaIds); $j++): ?>
                                    ...
                                    <?php for ($k = 0; $k < count($slaData); $k++): ?>
                                        <?php if (isset($slaData[$k]) && $slaData[$k]['slaId'] == $slaIds[$j]): ?>
                                            <span
                                                class="<?php if (($slaData[$k]['goalValue'] - $slaData[$k]['intervalMinutes']) < 0) {
                                                    echo 'slaNegative';
                                                } else echo 'slaPositive' ?>">
                                                <?php echo UbirimiContainer::get()['repository']->get(
                                                    Sla::class
                                                )->formatOffset(
                                                    $slaData[$k]['goalValue'] - $slaData[$k]['intervalMinutes']
                                                ) ?>
                                            </span>
