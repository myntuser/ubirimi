    ...
    <tr>
        <td>
            <select id="search_issue_status" name="search_issue_status[]" multiple="multiple" class="search_combobox">
                <option <?php if (!isset($getSearchParameters['status']) || in_array(
                        -1,
                        $getSearchParameters['status']
                    )
                ...
                <?php for ($i = 0; $i < count($searchCriteria['all_client_issue_status']); $i++): ?>
                    ...
                    ?>
                    <option
                        <?php if ($found) echo 'selected="selected"' ?>value="<?php echo $searchCriteria['all_client_issue_status'][$i]['id'] ?>"><?php echo $searchCriteria['all_client_issue_status'][$i]['name']; ?></option>
                <?php endfor ?>
            </select>
        </td>
    </tr>
