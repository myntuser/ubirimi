<table cellpadding="0" cellspacing="0" border="0">
    ...
    <tr>
        <td>
            <select id="search_affects_version" name="search_affects_version[]" multiple="multiple"
                    class="search_combobox">
                <option <?php if (!isset($getSearchParameters['affects_version']) || in_array(
                        -1,
                        $getSearchParameters['affects_version']
                    )
                ...
                <?php for ($i = 0; $i < count($searchCriteria['all_client_issue_version']); $i++): ?>
                    ...
                    <option <?php if ($found) echo 'selected="selected"' ?>
                        value="<?php echo $searchCriteria['all_client_issue_version'][$i]['id'] ?>"><?php echo $searchCriteria['all_client_issue_version'][$i]['name']; ?></option>
                <?php endfor ?>
            </select>
        </td>
    </tr>
