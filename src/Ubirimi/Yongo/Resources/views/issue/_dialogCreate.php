...
if ($issueSecuritySchemeId) {
    $issueSecuritySchemeLevels = UbirimiContainer::get()['repository']->get(
...
$issuePriorities = UbirimiContainer::get()['repository']->get(IssueSettings::class)->getAllIssueSettings(
    'priority',
    $clientId
);
$issueResolutions = UbirimiContainer::get()['repository']->get(IssueSettings::class)->getAllIssueSettings(
    'resolution',
    $clientId
);
...
$userHasModifyReporterPermission = UbirimiContainer::get()['repository']->get(YongoProject::class)->userHasPermission(
    $projectId,
    Permission::PERM_MODIFY_REPORTER,
    $loggedInUserId
);
$userHasAssignIssuePermission = UbirimiContainer::get()['repository']->get(YongoProject::class)->userHasPermission(
    $projectId,
    Permission::PERM_ASSIGN_ISSUE,
    $loggedInUserId
);
$userHasSetSecurityLevelPermission = UbirimiContainer::get()['repository']->get(YongoProject::class)->userHasPermission(
    $projectId,
    Permission::PERM_SET_SECURITY_LEVEL,
    $loggedInUserId
);
        ...
        switch ($field['field_code']) {
            ...
            case Field::FIELD_ASSIGNEE_CODE:
                echo IssueHelper::renderUserSelect(
                    ...
                    1 === UbirimiContainer::get()['repository']->get(UbirimiClient::class)->getYongoSetting(
                        $clientId,
...
for ($i = 0; $i < count($fieldData); $i++) {
    ...
    if (!in_array($fieldData[$i]['field_id'], $fieldsPlacedOnScreen) && $fieldData[$i]['required_flag']) {
        if ($fieldData[$i]['field_code'] == Field::FIELD_ISSUE_SECURITY_LEVEL_CODE) {
            ...
            $defaultLevel = UbirimiContainer::get()['repository']->get(IssueSecurityScheme::class)->getDefaultLevel(
                $projectData['issue_security_scheme_id']
            );
            echo '<tr>';
            echo '<td>';
            if (!$defaultLevel) {
