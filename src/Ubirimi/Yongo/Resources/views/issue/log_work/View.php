        ...
        <?php while ($workLogs && $workLog = $workLogs->fetch_array(MYSQLI_ASSOC)): ?>
            ...
            <tr>
                <td>
                    ...
                    <div>Time Spent: <?php echo Util::transformTimeToString(
                            Util::transformLogTimeToMinutes($workLog['time_spent'], $hoursPerDay, $daysPerWeek),
                            $hoursPerDay,
                            $daysPerWeek
                        ); ?></div>
                ...
                <td width="20px" align="right">
                    <?php if (($workLog['user_id'] == $loggedInUserId && $hasEditOwnWorklogsPermission) || $hasEditAllWorklogsPermission): ?>
                        <img style="cursor: pointer" id="edit_work_log_<?php echo $workLog['id'] ?>" title="Edit"
                             height="16px" src="/img/edit.png"/>
                    <?php endif ?>
                </td>
                <td width="20px" align="right">
                    <?php if (($workLog['user_id'] == $loggedInUserId && $hasDeleteOwnWorklogsPermission) || $hasDeleteAllWorklogsPermission): ?>
                        <img style="cursor: pointer" id="delete_work_log_<?php echo $workLog['id'] ?>" title="Delete"
