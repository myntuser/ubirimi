...
<table class="modal-table">
    <?php if ($mode != 'delete'): ?>
        ...
        <tr>
            ...
            <td>
                <input required="1" style="width: 140px" class="inputText" id="log_work_date_started" type="text"
                       value="<?php if ($workLog) {
                           echo $dateStartedString;
                       } else echo date('d-m-Y h:i') ?>"/>
    ...
    <tr>
        ...
        <td>
            <input checked="checked" type="radio" name="log_work_remaining_estimate"
                   id="radio_log_work_remaining_estimate_auto" value="adjust_automatically"/>
            <label for="radio_log_work_remaining_estimate_auto"><span>Adjust automatically</span></label>

            <div class="smallDescription" style="margin-left: 21px">the estimate will be reduced by the amount of work
                done, but never below 0.
            </div>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <?php if ($remainingEstimate != '-1'): ?>
                <input type="radio" name="log_work_remaining_estimate" value="existing_estimate"
                       id="radio_log_work_remaining_estimate"/>
                <label
                    for="radio_log_work_remaining_estimate"><span>Use existing estimate of <?php echo $remainingEstimate ?></span></label>
            <?php else: ?>
                <input type="radio" name="log_work_remaining_estimate" value="estimate_unset"
                       id="radio_log_work_remaining_estimate"/>
    ...
    <?php if ($mode == 'new'): ?>
        <tr>
            <td></td>
            <td>
                <input type="radio" name="log_work_remaining_estimate" value="reduce_by"
                       id="radio_log_work_remaining_reduce_by"/>
    ...
    <?php elseif ($mode == 'delete'): ?>
        <tr>
            <td></td>
            <td>
                <input type="radio" name="log_work_remaining_estimate" value="increase_by"
                       id="radio_log_work_remaining_increase_by"/>
        ...
        <tr>
            ...
            <td>
                <textarea class="inputTextAreaLarge"
                          id="log_work_work_description"><?php echo $workLog['comment'] ?></textarea>
            </td>
        </tr>
    <?php endif ?>
</table>