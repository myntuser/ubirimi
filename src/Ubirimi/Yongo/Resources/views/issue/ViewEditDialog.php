    ...
    if ($arrayData && $arrayData['visible_flag']) {
        ...
        switch ($field['field_code']) {
            ...
            case Field::FIELD_SUMMARY_CODE:
                ...
                echo '<input ' . $requiredHTML . ' id="field_type_' . $field['field_code'] . '" class="inputTextLarge mousetrap" type="text" value="' . htmlspecialchars(
                        $issueData['summary'],
                        ENT_QUOTES
            ...
            case Field::FIELD_AFFECTS_VERSION_CODE:
                if ($projectVersions) {
                    ...
                    while ($version = $projectVersions->fetch_array(MYSQLI_ASSOC)) {
                        $textSelected = '';
                        if (isset($arrayIssueVersionsAffected) && in_array(
                                $version['id'],
                                $arrayIssueVersionsAffected
                            )
                        ) {
                            $textSelected = 'selected="selected"';
                        }
            ...
            case Field::FIELD_FIX_VERSION_CODE:
                if ($projectVersions) {
                    ...
                    while ($version = $projectVersions->fetch_array(MYSQLI_ASSOC)) {
                        $textSelected = '';
                        if (isset($arrayIssueVersionsTargeted) && in_array(
                                $version['id'],
                                $arrayIssueVersionsTargeted
                            )
                        ) {
                            $textSelected = 'selected="selected"';
                        }
            ...
            case $fieldCodeNULL:
                ...
                switch ($field['type_code']) {
                    case Field::CUSTOM_FIELD_TYPE_SMALL_TEXT_CODE:
                        echo '<input ' . $requiredHTML . ' id="field_custom_type_' . $field['field_id'] . '_' . $field['type_code'] . '" class="inputTextLarge mousetrap" type="text" value="' . htmlspecialchars(
                                $fieldValue['value'],
                                ENT_QUOTES
                            ) . '" name="' . $field['type_code'] . '" />';
                        break;
