            ...
            <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
                <tr>
                    <?php if ($hasEditPermission && $issueEditableProperty): ?>
                        <td><a id="btnEditIssueFromDetail" href="#" class="btn ubirimi-btn"><i class="icon-edit"></i>
                                Edit</a></td>
                    <?php endif ?>
                    ...
                    <?php if ($hasAddCommentsPermission): ?>
                        <td>
                            <a id="btnEditIssueComment" href="#" class="btn ubirimi-btn"><i class="icon-comment"></i>
                                Comment</a>
                        </td>
                    <?php endif ?>
                    <td>
                        <?php
                        Util::includePartial(
                            __DIR__ . '/_moreActionMenu.php',
                            array(
                                'issueEditableProperty' => $issueEditableProperty,
                                'issue' => $issue,
                    ...
                    <td><a id="btnPrint" target="_blank" href="/yongo/issue/print/<?php echo $issue['id'] ?>"
                    ...
                    while ($workflowActions && $workflowStep = $workflowActions->fetch_array(MYSQLI_ASSOC)) {
                        ...
                        $transitionEvent = UbirimiContainer::get()['repository']->get(
                            IssueEvent::class
                        )->getEventByWorkflowDataId($workflowDataId);
                        ...
                        switch ($transitionEvent['code']) {
                            ...
                            case IssueEvent::EVENT_ISSUE_CLOSED_CODE:
                                ...
                                $hasEventPermission = UbirimiContainer::get()['repository']->get(
                                    YongoProject::class
                                )->userHasPermission($projectId, Permission::PERM_CLOSE_ISSUE, $loggedInUserId);
                                break;
                            ...
                            case IssueEvent::EVENT_ISSUE_RESOLVED_CODE:
                                $hasEventPermission = UbirimiContainer::get()['repository']->get(
                                    YongoProject::class
                                )->userHasPermission($projectId, Permission::PERM_RESOLVE_ISSUE, $loggedInUserId);
                                break;
                            ...
                            case IssueEvent::EVENT_ISSUE_WORK_STOPPED_CODE:
                                ...
                                $hasEventPermission = UbirimiContainer::get()['repository']->get(
                                    YongoProject::class
                                )->userHasPermission($projectId, Permission::PERM_EDIT_ISSUE, $loggedInUserId);
                                break;
                        ...
                        $canBeExecuted = UbirimiContainer::get()['repository']->get(
                            Workflow::class
                        )->checkConditionsByTransitionId($workflowStep['id'], $loggedInUserId, $issue);
                    ...
                    <td>
                        <div class="btn-group">
                            <ul class="dropdown-menu">
                                ...
                                <?php while ($workflowActions && $workflowStep = $workflowActions->fetch_array(
                                        MYSQLI_ASSOC
                                    )): ?>

                                    <?php $canBeExecuted = UbirimiContainer::get()['repository']->get(
                                        Workflow::class
                                    )->checkConditionsByTransitionId($workflowStep['id'], $loggedInUserId, $issue); ?>
                                    ...
                                    <li>
                                        <?php if ($workflowStep['screen_id']): ?>
                                            <?php if ($canBeExecuted && $hasEventPermission): ?>
                                                <a id="issue_transition_with_screen_<?php echo $step['id'] . '_' . $workflowStep['workflow_step_id_to']; ?>"
                                                   href="#"><?php echo $workflowStep['transition_name'] ?></a>
                                            <?php endif ?>
                                        <?php else: ?>
                                            <?php if ($canBeExecuted && $hasEventPermission): ?>
                                                <a id="issue_transition_with_no_screen_<?php echo $step['id'] . '_' . $workflowStep['workflow_step_id_to']; ?>"
                                                   href="#"><?php echo $workflowStep['transition_name'] ?></a>
                                            <?php endif ?>
                                        <?php endif ?>
                                    </li>
                            ...
                            <a class="btn ubirimi-btn dropdown-toggle <?php if (0 == $workflowMenuEnabled): ?>disabled<?php endif ?>"
        ...
        <td align="right">
            ...
            <?php if ($arrayListResultIds): ?>
                <?php if ($session->has('last_search_parameters')): ?>
                    <?php if ($session->get('selected_product_id') == SystemProduct::SYS_PRODUCT_YONGO): ?>
                        <a class="btn ubirimi-btn"
                           href="/yongo/issue/search?<?php echo $session->get('last_search_parameters') ?>">Return to
                            Search</a>
                    <?php else: ?>
                        <a class="btn ubirimi-btn" href="/helpdesk/customer-portal/tickets?<?php echo $session->get(
                ...
                <?php if (($index - 1) >= 0 && $arrayListResultIds[$index - 1]): ?>
                    ...
                    <?php else: ?>
                        <a class="btn ubirimi-btn"
                           href="/helpdesk/customer-portal/ticket/<?php echo $arrayListResultIds[$index - 1] ?>">
                            <img border="0" height="10" src="/img/br_prev.png"/> Previous</a>
                    <?php endif ?>
                <?php else: ?>
                    <a class="btn ubirimi-btn disabled" href="#"><img border="0" height="10" src="/img/br_prev.png"/>
                        Previous</a>
                <?php endif ?>
                <?php if ((($index + 1) < count($arrayListResultIds)) && $arrayListResultIds[$index + 1]): ?>
                    <?php if ($session->get('selected_product_id') == SystemProduct::SYS_PRODUCT_YONGO): ?>
                        <a class="btn ubirimi-btn" href="/yongo/issue/<?php echo $arrayListResultIds[$index + 1] ?>">Next
                            <img border="0" height="10" src="/img/br_next.png"/></a>
                    <?php else: ?>
                        <a class="btn ubirimi-btn"
                           href="/helpdesk/customer-portal/ticket/<?php echo $arrayListResultIds[$index + 1] ?>">Next
                            <img border="0" height="10" src="/img/br_next.png"/></a>
                    <?php endif ?>
                <?php else: ?>
                    <a class="btn ubirimi-btn disabled" href="#">Next <img border="0" height="10"
                                                                           src="/img/br_next.png"/></a>
                <?php endif ?>
            <?php endif ?>
        </td>
    </tr>
