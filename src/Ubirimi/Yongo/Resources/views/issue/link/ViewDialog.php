<?php if ($linkPossible): ?>
    ...
    <table width="100%" class="modal-table">
            ...
            <td>
                <select id="link_type" class="select2InputSmall">
                    <?php while ($linkType = $types->fetch_array(MYSQLI_ASSOC)): ?>
                        <option
                            value="<?php echo $linkType['id'] ?>_outward"><?php echo $linkType['outward_description'] ?></option>
                        <option
                            value="<?php echo $linkType['id'] ?>_inward"><?php echo $linkType['inward_description'] ?></option>
                    <?php endwhile ?>
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <select data-placeholder="Select an issue..." id="link_child_issue" name="link_child_issue"
                        multiple="multiple" class="select2Input" style="width: 400px;">
                    <?php while ($issue = $issues->fetch_array(MYSQLI_ASSOC)): ?>
                        <?php if ($issue['id'] != $issueId): ?>
                            <option
                                value="<?php echo $issue['id'] ?>"><?php echo $issue['project_code'] . '-' . $issue['nr']; ?></option>
                        <?php endif ?>
                    <?php endwhile ?>
                </select>
            </td>
