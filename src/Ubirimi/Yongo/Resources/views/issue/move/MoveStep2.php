...
<div class="pageContent">
    ...
    <table width="100%">
        <tr>
            ...
            <td width="15px"></td>
            <td valign="top">
                <form name="move_issue_step_2" method="post"
                      action="/yongo/issue/move/status/<?php echo $issue['id'] ?>">
                    <b>Step 2 of 4: Select the status of the issue</b>
                    <br>

                    <div>Current Issue (Workflow: <?php echo $currentWorkflow['name'] ?>
                        -> <?php echo $newWorkflow['name'] ?>)
                    </div>
                    <hr size="1"/>
                    <table width="100%">
                        <tr>
                            ...
                            <td width="25%">
                                <select name="move_to_status" class="inputTextCombo">
                                    <?php while ($newStatuses && $status = $newStatuses->fetch_array(MYSQLI_ASSOC)): ?>
                                        <option
                                            value="<?php echo $status['linked_issue_status_id'] ?>"><?php echo $status['name'] ?></option>
                                    <?php endwhile ?>
                                </select>
                            </td>
                        </tr>
