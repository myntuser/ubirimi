...
<div class="pageContent">
    ...
    <table width="100%">
        <tr>
            ...
            <td width="15px"></td>
            <td valign="top">
                <form name="move_issue_step_1_subtask" method="post"
                      action="/yongo/issue/move/subtask-issue-type/<?php echo $issue['id'] ?>">
                    <b>Step 1 of 4: Select Issue Types for Sub-Tasks</b>
                    <br/>
                    The table below lists all the sub-tasks that need to be moved to a new project. Please select the
                    appropriate issue type for each of them.
                    <br>
                    ...
                    <table width="100%">
                        <?php for ($i = 0; $i < count($oldSubtaskIssueType); $i++): ?>
                            <?php $subTask = UbirimiContainer::get()['repository']->get(IssueType::class)->getById(
                                $oldSubtaskIssueType[$i]
                            ); ?>
                            <tr>
                                ...
                                <td width="25%">
                                    <select name="new_subtask_issue_type_<?php echo $oldSubtaskIssueType[$i] ?>"
                                            class="inputTextCombo">
                                        <?php while ($newSubtaskIssueType && $issueType = $newSubtaskIssueType->fetch_array(
                                                MYSQLI_ASSOC
                                            )): ?>
                                            <option
                                                value="<?php echo $issueType['id'] ?>"><?php echo $issueType['name'] ?></option>
                                        <?php endwhile ?>
                                    </select>
