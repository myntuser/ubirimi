...
<div class="pageContent">
    <?php if (Util::userHasYongoAdministrativePermission()): ?>
        ...
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <td><a id="btnNew" href="/yongo/administration/screen/add" class="btn ubirimi-btn"><i
                            class="icon-plus"></i> Create Screen</a></td>
                <?php if ($screens): ?>
                    <td><a id="btnEditScreenMetaData" href="#" class="btn ubirimi-btn disabled"><i
                                class="icon-edit"></i> Edit</a></td>
                    <td><a id="btnEditScreen" href="#" class="btn ubirimi-btn disabled"><i class="icon-wrench"></i>
                            Configure</a></td>
                    <td><a id="btnCopyScreen" href="#" class="btn ubirimi-btn disabled">Copy</a></td>
                    <td><a id="btnDeleteScreen" href="#" class="btn ubirimi-btn disabled"><i class="icon-remove"></i>
                            Delete</a></td>
                <?php endif ?>
            </tr>
        </table>
        <div class="infoBox">
            <div>A screen is an arrangement of fields that are displayed when the issue is created, edited or
                transitioned through workflow.
            </div>
            <div>To choose screens that are displayed when issues are created or edited please map the screens to issue
                operations using <a href="/yongo/administration/screens/schemes">Screen Schemes</a>.
            </div>
            <div>To select which screen is displayed for a particular workflow transition, please select the <a
                    href="/yongo/administration/workflows">workflow</a> the transition belongs to and edit it.
            </div>
        </div>
        <?php if ($screens): ?>
            <table class="table table-hover table-condensed">
                ...
                <?php while ($screen = $screens->fetch_array(MYSQLI_ASSOC)): ?>
                    <tr id="table_row_<?php echo $screen['id'] ?>">
                        ...
                        <td>
                            <?php
                            $screenSchemes = UbirimiContainer::get()['repository']->get(
                                ScreenScheme::class
                            )->getByScreenId($clientId, $screen['id']);
                            if ($screenSchemes) {
                                echo '<ul>';
                        ...
                        <td width="500px">
                            <?php
                            $workflows = UbirimiContainer::get()['repository']->get(Workflow::class)->getByScreen(
                                $clientId,
                                $screen['id']
                            );
                            if ($workflows) {
