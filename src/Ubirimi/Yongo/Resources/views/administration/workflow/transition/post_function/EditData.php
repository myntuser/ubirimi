...
<div class="pageContent">
    ...
    <form name="edit_post_function_data"
          action="/yongo/administration/workflow/edit-post-function-data/<?php echo $workflowPostFunctionDataId; ?>"
          method="post">
        <?php if ($postFunctionId == WorkflowFunction::FUNCTION_SET_ISSUE_FIELD_VALUE): ?>
            <?php $resolutions = UbirimiContainer::get()['repository']->get(IssueSettings::class)->getAllIssueSettings(
                'resolution',
                $clientId
            ); ?>
            ...
            <table width="100%">
                ...
                <tr>
                    ...
                    <td>
                        <select name="field_value" class="select2InputSmall">
                            <option value="-1" <?php if ($fieldValue == -1) echo 'selected="selected"' ?>>Clear Value
                            </option>
                            <?php while ($resolution = $resolutions->fetch_array(MYSQLI_ASSOC)): ?>
                                <option  <?php if ($fieldValue == $resolution['id']) echo 'selected="selected"' ?>
                                    value="<?php echo $resolution['id'] ?>"><?php echo $resolution['name'] ?></option>
                            <?php endwhile ?>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td align="left">
                        <div align="left">
                            <button type="submit" name="edit_parameters" class="btn ubirimi-btn"><i
                                    class="icon-edit"></i> Update Parameters
                            </button>
                            <a class="btn ubirimi-btn"
                               href="/yongo/administration/workflow/transition-post-functions/<?php echo $workflowPostFunctionData['workflow_data_id'] ?>">Cancel</a>
                        </div>
                    </td>
                </tr>
            </table>
        <?php elseif ($postFunctionId == WorkflowFunction::FUNCTION_FIRE_EVENT): ?>
            ...
            <table width="100%">
                <tr>
                    ...
                    <td>
                        <?php $events = UbirimiContainer::get()['repository']->get(IssueEvent::class)->getByClient(
                            $clientId
                        ); ?>
        ...
        <input type="hidden" name="workflow_data_id"
               value="<?php echo $workflowPostFunctionData['workflow_data_id'] ?>"/>
    </form>
</div>
