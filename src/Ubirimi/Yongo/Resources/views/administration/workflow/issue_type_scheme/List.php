...
<div class="pageContent">
    <?php if (Util::userHasYongoAdministrativePermission()): ?>
        ...
        <ul class="nav nav-tabs" style="padding: 0px;">
            ...
            <li class="active"><a href="/yongo/administration/workflows/issue-type-schemes">Workflow Issue Type
                    Schemes</a></li>
        ...
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <td><a id="btnNew" href="/yongo/administration/issue/add-type-scheme/workflow"
                       class="btn ubirimi-btn"><i class="icon-plus"></i> Create Workflow Issue Type Scheme</a></td>
                <td><a id="btnEditIssueTypeScheme" href="" class="btn ubirimi-btn disabled"><i class="icon-edit"></i>
                        Edit</a></td>
                <td><a id="btnCopyWorkflowIssueTypeScheme" href="/yongo/administration/issue-type-scheme/copy/3"
                       class="btn ubirimi-btn disabled">Copy</a></td>
                <td><a id="btnDeleteWorkflowIssueTypeScheme" href="#" class="btn ubirimi-btn"><i
                            class="icon-remove"></i> Delete</a></td>
            </tr>
        </table>
        <div class="infoBox">
            <div>Workflow Issue Type Schemes allow you to set what issue types will be used by a specific workflow.
            </div>
        </div>
        <?php if ($issueTypeSchemes): ?>
            <table class="table table-hover table-condensed">
                ...
                <tr id="table_row_<?php echo $scheme['id'] ?>">
                    ...
                    <td width="300px">
                        <?php $schemeIssueTypes = UbirimiContainer::get()['repository']->get(
                            IssueTypeScheme::class
                        )->getDataById($scheme['id']) ?>
                        <ul>
                            <?php while ($schemeIssueTypes && $issueType = $schemeIssueTypes->fetch_array(
                                    MYSQLI_ASSOC
                                )): ?>
                    ...
                    <td>
                        <?php $projects = UbirimiContainer::get()['repository']->get(
                            YongoProject::class
                        )->getByWorkflowIssueTypeScheme($clientId, $scheme['id']) ?>
                        <?php if ($projects): ?>
                            <ul>
