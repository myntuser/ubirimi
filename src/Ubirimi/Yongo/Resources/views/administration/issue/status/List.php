...
<div class="pageContent">
    <?php if (Util::userHasYongoAdministrativePermission()): ?>
        ...
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <td><a id="btnNew" href="/yongo/administration/issue/status/add" class="btn ubirimi-btn"><i
                            class="icon-plus"></i> Create Status</a></td>
                <?php if ($statuses): ?>
                    <td><a id="btnEditIssueStatus" href="#" class="btn ubirimi-btn disabled"><i class="icon-edit"></i>
                            Edit</a></td>
                    <td><a id="btnDeleteIssueStatus" href="#" class="btn ubirimi-btn disabled"><i
                                class="icon-remove"></i> Delete</a></td>
                <?php endif ?>
            </tr>
        </table>
        <div class="infoBox">
            <div>All statuses have one of two modes: Active - associated with a workflow/s. Inactive - not in use. To
                delete a status, it must not be associated with a workflow.
            </div>
        </div>
            ...
            <table class="table table-hover table-condensed">
                ...
                <?php while ($status = $statuses->fetch_array(MYSQLI_ASSOC)): ?>
                    <tr id="table_row_<?php echo $status['id'] ?>">
                        ...
                        <td>
                            <?php $workflows = UbirimiContainer::get()['repository']->get(
                                Workflow::class
                            )->getByIssueStatusId($status['id']); ?>
                            <?php if ($workflows): ?>
                                <div>Active</div>
                            <?php else: ?>
                                <div>Inactive</div>
                            <?php endif ?>
                            <input type="hidden" id="delete_possible_<?php echo $status['id'] ?>"
                                   value="<?php if ($workflows) {
                                       echo "0";
                                   } else echo "1" ?>"/>
                        </td>
                        <td width="500px">
