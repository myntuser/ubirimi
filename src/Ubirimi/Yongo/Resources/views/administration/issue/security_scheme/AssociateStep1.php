...
<div class="pageContent">
    ...
    <form name="associate" method="post"
          action="/yongo/administration/project/associate-issue-security-scheme/<?php echo $projectId ?>">
        <table width="100%">
            <tr>
                ...
                <td>
                    <select class="inputTextCombo" name="scheme">
                        <?php while ($issueSecurityScheme = $issueSecuritySchemes->fetch_array(MYSQLI_ASSOC)): ?>
                            <option
                                value="<?php echo $issueSecurityScheme['id'] ?>"><?php echo $issueSecurityScheme['name'] ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            </tr>
