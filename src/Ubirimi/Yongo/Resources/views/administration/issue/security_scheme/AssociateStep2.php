        ...
        <tr>
            ...
            <td>
                <div class="headerPageText">
                    <a href="/yongo/administration/projects" class="linkNoUnderline">Projects</a>
                    > <?php echo $project['name'] ?> > Associate Issue Security
                </div>
            </td>
        </tr>
    </table>
...
<div class="pageContent">
    ...
    <form name="associate" method="post"
        ...
        <table width="100%">
            ...
            if ($projectIssueSecurityScheme) {
                $header .= $projectIssueSecurityScheme['name'] . '</b></div>';
            }
            $header .= '</td>';
            $header .= '<td>';
            $header .= '<div><b>Security Levels for ';
            if ($selectedScheme) {
                $header .= $selectedScheme['name'] . '</b></div>';
            }
            $header .= '</td>';
            $header .= '</tr>';
            ?>
            ...
            if ($projectIssueSecuritySchemeId) {
                $projectLevels = UbirimiContainer::get()['repository']->get(
                    IssueSecurityScheme::class
                )->getLevelsByIssueSecuritySchemeId($projectIssueSecuritySchemeId);
            }
            ...
            if ($projectLevels) {
                while ($projectLevel = $projectLevels->fetch_array(MYSQLI_ASSOC)) {
                    $issues = UbirimiContainer::get()['repository']->get(Issue::class)->getByParameters(
                        array('project' => $projectId, 'security_scheme_level' => $projectLevel['id'])
                    );
                    if ($issues) {
                ...
                $issues = UbirimiContainer::get()['repository']->get(
                    YongoProject::class
                )->getIssuesWithNoSecurityScheme($projectId);
