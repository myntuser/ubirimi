...
if ('workflow' == $type) {
    Util::renderBreadCrumb(
        '<a class="linkNoUnderline" href="/yongo/administration/workflows/issue-type-schemes">Workflow Issue Type Schemes</a> > ' . $issueTypeScheme['name'] . ' > Copy'
    );
} else {
    Util::renderBreadCrumb(
        '<a class="linkNoUnderline" href="/yongo/administration/issue-type-schemes">Issue Type Schemes</a> > ' . $issueTypeScheme['name'] . ' > Copy'
    );
}
?>
<div class="pageContent">
    <form name="form_copy_issue_type_scheme"
          action="/yongo/administration/issue-type-scheme/copy/<?php echo $issueTypeSchemeId ?>?type=<?php echo $type ?>"
          method="post">
        ...
        <table width="100%">
            ...
            <tr>
                ...
                <td>
                    <textarea class="inputTextAreaLarge"
                              name="description"><?php echo $issueTypeScheme['description'] ?></textarea>
                </td>
            </tr>
            <tr>
                <td colspan="2">
            ...
            <tr>
                ...
                <td>
                    <?php if ('workflow' == $type): ?>
                        <button type="submit" name="copy_issue_type_scheme" class="btn ubirimi-btn">Copy Workflow Issue
                            Type Scheme
                        </button>
                        <a class="btn ubirimi-btn" href="/yongo/administration/workflows/issue-type-schemes">Cancel</a>
                    <?php else: ?>
                        <button type="submit" name="copy_issue_type_scheme" class="btn ubirimi-btn">Copy Issue Type
                            Scheme
                        </button>
