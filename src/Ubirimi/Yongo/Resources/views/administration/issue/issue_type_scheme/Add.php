...
if ('workflow' == $type) {
    Util::renderBreadCrumb(
        '<a href="/yongo/administration/workflows/issue-type-schemes" class="linkNoUnderline">Workflow Issue Type Schemes</a> > Create Workflow Issue Type Scheme'
    );
} else {
    Util::renderBreadCrumb(
        '<a href="/yongo/administration/issue-type-schemes" class="linkNoUnderline">Issue Type Schemes</a> > Create Issue Type Scheme'
    );
}
?>
    ...
    <form name="add_status" action="/yongo/administration/issue/add-type-scheme/<?php echo $type ?>" method="post">
        ...
        <table width="100%">
            ...
            <?php while ($issueType = $allIssueTypes->fetch_array(MYSQLI_ASSOC)): ?>
                <tr>
                    <td colspan="2">
                        <input type="checkbox" value="1" id="issue_type_<?php echo $issueType['id'] ?>"
                               name="issue_type_<?php echo $issueType['id'] ?>"/>
                            <span>
                                <label
                                    for="issue_type_<?php echo $issueType['id'] ?>"><?php echo $issueType['name'] ?></label>
                            </span>
                    </td>
                </tr>
            <?php endwhile ?>
            ...
            <tr>
                <td></td>
                <td>
                    <button type="submit" name="new_type_scheme"
                            class="btn ubirimi-btn"><?php echo $buttonLabel ?></button>
