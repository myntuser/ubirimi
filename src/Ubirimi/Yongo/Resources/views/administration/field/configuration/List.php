...
<div class="pageContent">
    ...
    <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
        <tr>
            <td><a id="btnNew" href="/yongo/administration/field-configuration/add" class="btn ubirimi-btn"><i
                        class="icon-plus"></i> Create Field Configuration</a></td>
            <?php if ($fieldConfigurations): ?>
                <td><a id="btnEditFieldConfigurationMetadata" href="#" class="btn ubirimi-btn disabled"><i
                            class="icon-edit"></i> Edit</a></td>
                <td><a id="btnEditFieldConfiguration" href="#" class="btn ubirimi-btn disabled"><i
                            class="icon-wrench"></i> Configure</a></td>
                <td><a id="btnCopyFieldConfiguration" href="#" class="btn ubirimi-btn disabled">Copy</a></td>
                <td><a id="btnDeleteFieldConfiguration" href="#" class="btn ubirimi-btn disabled"><i
                            class="icon-remove"></i> Delete</a></td>
            <?php endif ?>
        </tr>
    </table>
    <div class="infoBox">
        A Field Configuration provides the ability to change field behavior, it essentially tells Ubirimi how to handle
        a particular field. For example, a Field Configuration can be used to hide a field from all input screens and
        views, or to make a field require a value every time it is edited.
    </div>
    <?php if ($fieldConfigurations): ?>
        <table class="table table-hover table-condensed">
            ...
            <?php while ($fieldConfiguration = $fieldConfigurations->fetch_array(MYSQLI_ASSOC)): ?>
                <tr id="table_row_<?php echo $fieldConfiguration['id'] ?>">
                    ...
                    <td width="500px">
                        <?php $schemes = UbirimiContainer::get()['repository']->get(
                            FieldConfigurationScheme::class
                        )->getFieldConfigurationsSchemesByFieldConfigurationId($clientId, $fieldConfiguration['id']) ?>
                        <?php if ($schemes): ?>
                            <ul>
                                <?php while ($scheme = $schemes->fetch_array(MYSQLI_ASSOC)): ?>
                                    <li>
                                        <a href="/yongo/administration/field-configuration/scheme/edit/<?php echo $scheme['id'] ?>"><?php echo $scheme['name'] ?></a>
                                    </li>
                                <?php endwhile ?>
                            </ul>
                            <input type="hidden" id="delete_possible_<?php echo $fieldConfiguration['id'] ?>"
                                   value="0"/>
                        <?php else: ?>
                            <input type="hidden" id="delete_possible_<?php echo $fieldConfiguration['id'] ?>"
                                   value="1"/>
