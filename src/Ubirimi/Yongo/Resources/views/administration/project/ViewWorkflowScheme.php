...
<div class="pageContent">
    ...
    <?php if ($session->get('user/super_user_flag')): ?>
        <?php if ($hasGlobalAdministrationPermission || $hasGlobalSystemAdministrationPermission): ?>
            <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
                <tr>
                    <td>
                        <a href="/yongo/administration/project/workflows/select-project-workflow-scheme/<?php echo $projectId ?>"
    ...
    <?php while ($workflow = $workflows->fetch_array(MYSQLI_ASSOC)): ?>
        <table width="100%">
            <tr>
                <td colspan="2">
                    <div class="headerPageText"><?php echo $workflow['name'] ?></div>
                </td>
            </tr>
            <tr>
                <td valign="top" width="180">
                    <?php $issueTypes = UbirimiContainer::get()['repository']->get(IssueTypeScheme::class)->getDataById(
                        $workflow['issue_type_scheme_id']
                    ); ?>
                ...
                <td valign="top">
                    ...
                    <table class="table table-hover table-condensed">
                        ...
                        <?php $steps = UbirimiContainer::get()['repository']->get(Workflow::class)->getSteps(
                            $workflow['id']
                        ) ?>
                        <?php while ($step = $steps->fetch_array(MYSQLI_ASSOC)): ?>
                            <tr>
                                ...
                                <td align="left">
                                    <?php
                                    $transitions = UbirimiContainer::get()['repository']->get(
                                        Workflow::class
                                    )->getTransitionsForStepId($workflow['id'], $step['id']); ?>
                                    <?php if ($transitions): ?>
                                        <?php for ($i = 0; $i < $transitions->num_rows; $i++): ?>
                                            <?php $transition = $transitions->fetch_array(MYSQLI_ASSOC) ?>
                                            <div <?php if ($i < $transitions->num_rows - 1) echo 'class="tdElementList"' ?>><?php echo $transition['transition_name'] ?>
