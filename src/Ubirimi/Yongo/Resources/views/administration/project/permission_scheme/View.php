...
<div class="headerPageBackground">
    <table width="100%">
        <tr>
            ...
            <td>
                <div class="headerPageText"><a href="/yongo/administration/projects"
                                               class="linkNoUnderline">Projects</a> > <?php echo $project['name'] ?> >
                    Permissions > <?php echo $permissionScheme['name'] ?></div>
            </td>
            <td align="right">
...
<div class="pageContent">
    ...
    <ul class="nav nav-tabs" style="padding: 0px;">
        ...
        <li class="active"><a href="/yongo/administration/project/permissions/<?php echo $projectId ?>">Permissions</a>
        </li>
    ...
    <?php if ($hasGlobalAdministrationPermission || $hasGlobalSystemAdministrationPermission): ?>
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <td>
                    <a href="/yongo/administration/permission-scheme/edit/<?php echo $permissionScheme['id'] ?>?back=view_project_permission&project_id=<?php echo $projectId ?>"
                       class="btn ubirimi-btn"><i class="icon-edit"></i> Edit Permissions</a></td>
                <td>
                    <a href="/yongo/administration/project/permissions/select-project-permission-scheme/<?php echo $projectId ?>"
                       class="btn ubirimi-btn">Use a Different Scheme</a></td>
            </tr>
        </table>
    <?php endif ?>
    <div class="infoBox">
        <div>Project permissions allow you to control who can access your project, and what they can do, e.g. "Work on
            Issues". Access to individual issues is granted to people by issue permissions.
        </div>
        <div>The permission scheme defines how the permissions are configured for this project. To change the
            permisisons, you can select a different permission scheme, or modify the currently selected scheme.
        </div>
    </div>
    <?php while ($category = $permissionCategories->fetch_array(MYSQLI_ASSOC)): ?>
        <table class="table table-hover table-condensed">
            ...
            <?php $permissions = UbirimiContainer::get()['repository']->get(Permission::class)->getByCategory(
                $category['id']
            ); ?>
            <?php while ($permission = $permissions->fetch_array(MYSQLI_ASSOC)): ?>
                <tr>
                    ...
                    <td>
                        <?php
                        $permData = UbirimiContainer::get()['repository']->get(
                            PermissionScheme::class
                        )->getDataByPermissionId($permissionScheme['id'], $permission['id']);
