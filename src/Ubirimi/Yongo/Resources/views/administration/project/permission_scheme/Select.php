...
<div class="pageContent">
    ...
    <form name="select_perm_scheme" method="post"
          action="/yongo/administration/project/permissions/select-project-permission-scheme/<?php echo $projectId ?>">
        <table width="100%">
            <tr>
                ...
                <td>
                    <select name="perm_scheme" class="select2InputMedium">
                        <?php while ($permissionScheme = $permissionSchemes->fetch_array(MYSQLI_ASSOC)): ?>
                            <option
                                value="<?php echo $permissionScheme['id'] ?>"><?php echo $permissionScheme['name'] ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            </tr>
