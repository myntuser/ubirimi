...
if ($forHelpDesk) {
    Util::renderBreadCrumb(
        '<a class="linkNoUnderline" href="/yongo/administration/projects">Projects</a> > Create Helpdesk Project'
    );
} else {
    Util::renderBreadCrumb(
        '<a class="linkNoUnderline" href="/yongo/administration/projects">Projects</a> > Create Project'
    );
}
?>

<div class="pageContent">
    <form name="add_project" action="/yongo/administration/project/add<?php if ($forHelpDesk) echo '?helpdesk=true' ?>"
          method="post">
        ...
        <table width="100%">
            <tr>
                ...
                <td>
                    <input id="project_name_add" class="inputText" type="text"
                           value="<?php if (isset($name)) echo $name ?>" name="name"/>
                    <?php if ($emptyName): ?>
                        <br/>
            ...
            <tr>
                ...
                <td>
                    <input style="text-transform: uppercase;" maxlength="5" class="inputText" type="text"
                           value="<?php if (isset($code)) echo $code ?>" name="code"/>
                    <?php if ($emptyCode): ?>
                        <br/>
                ...
                <td>
                    <select name="issue_type_screen_scheme" class="select2InputSmall">
                        <?php while ($screenScheme = $issueTypeScreenScheme->fetch_array(MYSQLI_ASSOC)): ?>
                            <option
                                value="<?php echo $screenScheme['id'] ?>"><?php echo $screenScheme['name']; ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <select name="field_configuration_scheme" class="select2InputSmall">
                        <?php while ($fieldConfigurationScheme = $fieldConfigurationSchemes->fetch_array(
                            MYSQLI_ASSOC
                        )): ?>
                            <option
                                value="<?php echo $fieldConfigurationScheme['id'] ?>"><?php echo $fieldConfigurationScheme['name']; ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td align="left">
                    <div align="left">
                        <button type="submit" name="confirm_new_project" class="btn ubirimi-btn"><i
                                class="icon-plus"></i> Create Project
                        </button>
                        <?php if ($forHelpDesk): ?>
