    ...
    <form name="add_project" action="/yongo/administration/project/edit/<?php echo $projectId ?>" method="post">
        <table width="100%">
            <tr>
                ...
                <td>
                    <input class="inputText" type="text" value="<?php if ($project && $project['name']) {
                        echo $project['name'];
                    } ?>" name="name"/>
                    <?php if ($emptyName): ?>
                        <br/>
            ...
            <tr>
                ...
                <td>
                    <input class="inputText" type="text" value="<?php if ($project && $project['code']) {
                        echo $project['code'];
                    } ?>" name="code"/>
                    <?php if ($emptyCode): ?>
                        <br/>
            ...
            <tr>
                ...
                <td>
                    <textarea class="inputTextAreaLarge"
                              name="description"><?php if ($project && $project['description']) {
                ...
                <td>
                    <select name="issue_type_scheme" class="select2InputSmall">
                        <?php while ($scheme = $issueTypeScheme->fetch_array(MYSQLI_ASSOC)): ?>
                            <option <?php if ($scheme['id'] == $project['issue_type_scheme_id']) echo 'selected="selected"' ?>
                                value="<?php echo $scheme['id'] ?>"><?php echo $scheme['name']; ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <select name="project_category" class="select2InputSmall">
                        <option <?php if (null == $project['project_category_id']) echo 'selected="selected"' ?>
                            value="-1">None
                        </option>
                        <?php if ($projectCategories): ?>
                            <?php while ($category = $projectCategories->fetch_array(MYSQLI_ASSOC)): ?>
                                <option <?php if ($category['id'] == $project['project_category_id']) echo 'selected="selected"' ?>
                ...
                <td>
                    <select name="workflow_scheme" class="select2InputSmall">
                        <?php while ($scheme = $workflowScheme->fetch_array(MYSQLI_ASSOC)): ?>
                            <option <?php if ($scheme['id'] == $project['workflow_scheme_id']) echo 'selected="selected"' ?>
                                value="<?php echo $scheme['id'] ?>"><?php echo $scheme['name']; ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            ...
            <tr>
                ...
                <td align="left">
                    <div align="left">
                        <button type="submit" name="confirm_edit_project" class="btn ubirimi-btn"><i
                                class="icon-edit"></i> Update Project
                        </button>
