...
<div class="pageContent">
    ...
    <form name="select_workflow_scheme" method="post"
          action="/yongo/administration/project/workflows/select-project-workflow-scheme/<?php echo $projectId ?>">
        <table width="100%">
            <tr>
                ...
                <td>
                    <select name="workflow_scheme" class="select2InputMedium">
                        <?php while ($workflowScheme = $workflowSchemes->fetch_array(MYSQLI_ASSOC)): ?>
                            <option <?php if ($project['workflow_scheme_id'] == $workflowScheme['id']) echo 'selected="selected"' ?>
                                value="<?php echo $workflowScheme['id'] ?>"><?php echo $workflowScheme['name'] ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            </tr>
