        ...
        <tr>
            ...
            <td>
                <div class="headerPageText"><a href="/yongo/administration/projects"
                    ...
                    Issue Security > <?php if ($issueSecurityScheme) {
                        echo $issueSecurityScheme['name'];
                    } else echo 'Anyone' ?></div>
            </td>
            <td align="right">
...
<div class="pageContent">
    ...
    <ul class="nav nav-tabs" style="padding: 0px;">
        ...
        <li class="active"><a href="/yongo/administration/project/issue-security/<?php echo $projectId ?>">Issue
                Security</a></li>
    ...
    <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
        <tr>
            <?php if ($issueSecurityScheme): ?>
                <td>
                    <a href="/yongo/administration/issue-security-scheme-levels/<?php echo $issueSecurityScheme['id'] ?>"
                       class="btn ubirimi-btn"><i class="icon-edit"></i> Edit Issue Security</a></td>
                <td><a href="/yongo/administration/project/associate-issue-security-scheme/<?php echo $projectId ?>"
                       class="btn ubirimi-btn">Use a Different Scheme</a></td>
            <?php else: ?>
                <td><a href="/yongo/administration/project/associate-issue-security-scheme/<?php echo $projectId ?>"
                       class="btn ubirimi-btn">Select a scheme</a></td>
            <?php endif ?>
        </tr>
    </table>
    ...
    <div class="infoBox">
        ...
        which can have users/groups assigned to them. The issue security scheme defines how the securities are
        configured for this project. To change the securities, you can select a different issue secuirty scheme, or
        modify the currently selected scheme.
    </div>
    ...
    <table class="table table-hover table-condensed">
        ...
        <?php if ($issueSecurityScheme): ?>
            <?php $issueSecuritySchemeLevels = UbirimiContainer::get()['repository']->get(
                IssueSecurityScheme::class
            )->getLevelsByIssueSecuritySchemeId($issueSecurityScheme['id']); ?>
            <tbody>
            <?php while ($level = $issueSecuritySchemeLevels->fetch_array(MYSQLI_ASSOC)): ?>
                <tr>
                    ...
                    <td>
                        <?php
                        $notificationData = UbirimiContainer::get()['repository']->get(
                            IssueSecurityScheme::class
                        )->getDataByLevelId($level['id']);
                        if ($notificationData) {
                            echo '<ul>';
