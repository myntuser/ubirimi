...
<div class="pageContent">
    <form name="select_issue_type_screen_scheme" method="post"
          action="/yongo/administration/project/select-issue-type-screen-scheme/<?php echo $projectId ?>">
        <table width="100%">
            <tr>
                ...
                <td>
                    <select name="issue_type_screen_scheme" class="select2InputMedium">
                        <?php while ($issueTypeScreenScheme = $issueTypeScreenSchemes->fetch_array(MYSQLI_ASSOC)): ?>
                            <option <?php if ($issueTypeScreenScheme['id'] == $project['issue_type_screen_scheme_id']) echo 'selected="selected"' ?>
                                value="<?php echo $issueTypeScreenScheme['id'] ?>"><?php echo $issueTypeScreenScheme['name'] ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            </tr>
