...
<div class="pageContent">
    ...
    <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
        <tr>
            <td><a id="btnNew" href="/yongo/administration/add-event" class="btn ubirimi-btn"><i class="icon-plus"></i>
                    Create New Event</a></td>
            <td><a id="btnEditEvent" href="#" class="btn ubirimi-btn disabled"><i class="icon-edit"></i> Edit</a></td>
            <td><a id="btnDeleteEvent" href="#" class="btn ubirimi-btn disabled"><i class="icon-remove"></i> Delete</a>
            </td>
        </tr>
    </table>
    ...
    <table class="table table-hover table-condensed">
        ...
        <?php while ($event = $events->fetch_array(MYSQLI_ASSOC)): ?>
            <tr id="table_row_<?php echo $event['id'] ?>">
                <td width="22">
                    <input <?php if ($event['system_flag']) echo 'disabled="disabled"' ?> type="checkbox" value="1"
                                                                                          id="el_check_<?php echo $event['id'] ?>"/>
                </td>
                <td>
                    <?php
                    echo $event['name'];
                    if ($event['system_flag'] == 1) {
                        echo ' (System)';
                    }
                    ?>
                </td>
                ...
                <td>
                    ...
                    $notificationSchemes = UbirimiContainer::get()['repository']->get(
                        IssueEvent::class
                    )->getNotificationSchemesByEventId($clientId, $event['id']);
                ...
                <td>
                    <?php
                    $workflows = UbirimiContainer::get()['repository']->get(IssueEvent::class)->getWorkflowsByEventId(
                        $clientId,
                        $event['id']
                    );
                    if ($workflows) {
                ...
                <input type="hidden" value="<?php echo $canBeDeleted ?>"
                       id="delete_possible_<?php echo $event['id'] ?>"/>
            </tr>
        <?php endwhile ?>
        </tbody>
    </table>
