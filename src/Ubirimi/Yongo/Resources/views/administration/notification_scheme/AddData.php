            ...
            <td>
                <div class="headerPageText">
                    <a class="linkNoUnderline" href="/yongo/administration/notification-schemes">Notification
                        Schemes</a> > <a class="linkNoUnderline"
                                         href="/yongo/administration/notification-scheme/edit/<?php echo $notificationSchemeId ?>"><?php echo $notificationScheme['name'] ?></a>
                    > Create Notification
                </div>
            </td>
            <td align="right">
                <div align="right">
                    <?php if (isset($backLink)): ?>
                        <a class="btn ubirimi-btn"
                           href="/yongo/administration/project/notifications/<?php echo $projectId ?>">Back</a>
                    <?php else: ?>
...
<div class="pageContent">
    ...
    <form name="add_notification_event_data"
          action="/yongo/administration/notification-scheme/add-data/<?php echo $notificationSchemeId ?>" method="post">
        <table width="100%">
            <tr>
                ...
                <td width="200">
                    <select class="inputTextCombo" name="event[]" size="10" multiple="multiple">
                        <?php while ($event = $events->fetch_array(MYSQLI_ASSOC)): ?>
                            <option <?php if ($event['id'] == $eventId) echo 'selected="selected"' ?>
                                value="<?php echo $event['id'] ?>"><?php echo $event['name'] ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
                <td></td>
                ...
                <td>
                    <select name="user" class="select2InputMedium" id="not_choose_user">
                        ...
                        <?php while ($user = $users->fetch_array(MYSQLI_ASSOC)): ?>
                            <option
                                value="<?php echo $user['id'] ?>"><?php echo $user['first_name'] . ' ' . $user['last_name'] . ' (' . $user['username'] . ')' ?></option>
                        <?php endwhile ?>
                    </select>
                </td>
            </tr>
            ...
            <?php if ($fieldsUserPickerMultipleSelection): ?>
                <tr>
                    <td></td>
                    <td>
                        <input class="radio" type="radio" name="type" id="label_user_picker_multiple_selection"
                               value="user_picker_multiple_selection">
                        <label for="label_user_picker_multiple_selection">User Custom Field Value</label>
                    </td>
                    <td>
                        <select name="select_user_picker_multiple_selection"
                                id="not_select_user_picker_multiple_selection" class="select2InputMedium">
            ...
            <tr>
                ...
                <td align="left">
                    <div align="left">
                        ...
                        <a class="btn ubirimi-btn"
                           href="/yongo/administration/notification-scheme/edit/<?php echo $notificationSchemeId ?>">Cancel</a>
                    </div>
                </td>
                <td></td>
            </tr>
