    ...
    <?php if ($groups): ?>
        ...
        <?php while ($group = $groups->fetch_array(MYSQLI_ASSOC)): ?>
            <tr id="table_row_<?php echo $group['id'] ?>">
                ...
                <td width="10%">
                    <?php
                    $usersInGroup = UbirimiContainer::get()['repository']->get(UbirimiGroup::class)->getDataByGroupId(
                        $group['id']
                    );
                    if ($usersInGroup) {
                ...
                <td width="20%">
                    <?php
                    $permissionSchemes = UbirimiContainer::get()['repository']->get(
                        PermissionScheme::class
                    )->getByClientIdAndGroupBy($clientId, $group['id']);
