...
class IssueFilter
{
    ...
    {
        $isFavourite = UbirimiContainer::get()['repository']->get(IssueFilter::class)->checkFilterIsFavouriteForUserId(
            $filterId,
            $userId
        );
        if ($isFavourite) {
            UbirimiContainer::get()['repository']->get(IssueFilter::class)->deleteFavouriteByFilterIdAndUserId(
                $userId,
                $filterId
            );
        } else {
