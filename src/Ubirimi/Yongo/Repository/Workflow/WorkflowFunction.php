...
class WorkflowFunction
{
    ...
    public function triggerPostFunctions(
        $clientId,
        $issueData,
        $workflowData,
        $issueFieldChanges,
    ...
    )
    {
        ...
        while ($functions && $function = $functions->fetch_array(MYSQLI_ASSOC)) {
            ...
            if ($function['sys_workflow_post_function_id'] == WorkflowFunction::FUNCTION_UPDATE_ISSUE_CHANGE_HISTORY) {
                UbirimiContainer::get()['repository']->get(Issue::class)->updateHistory(
                    $issueId,
                    $loggedInUserId,
                    $issueFieldChanges,
                    $currentDate
                    ...
                    if ($value[0] == Field::FIELD_RESOLUTION_CODE) {
                        if ($value[2]) {
                            UbirimiContainer::get()['repository']->get(Issue::class)->updateById(
                                $issueId,
                                array('date_resolved' => $currentDate),
                                $currentDate
                            );
                        } else {
            ...
            if ($function['sys_workflow_post_function_id'] == WorkflowFunction::FUNCTION_SET_ISSUE_STATUS_AS_IN_WORKFLOW_STEP) {
                ...
                $finalStatusId = UbirimiContainer::get()['repository']->get(Workflow::class)->getStepById(
                    $workflowData['workflow_step_id_to'],
                    'linked_issue_status_id'
                );

                $finalStatusName = UbirimiContainer::get()['repository']->get(IssueSettings::class)->getById(
                    $finalStatusId,
                    'status',
                    'name'
                );
                $initialStatusName = UbirimiContainer::get()['repository']->get(IssueSettings::class)->getById(
                    $issueData[Field::FIELD_STATUS_CODE],
                    'status',
                    'name'
                );
                $issueFieldChanges[] = array(
            ...
            if ($function['sys_workflow_post_function_id'] == WorkflowFunction::FUNCTION_SET_ISSUE_FIELD_VALUE) {
                ...
                switch ($field_name_data_arr[1]) {
                    case Field::FIELD_RESOLUTION_CODE:
                        ...
                        UbirimiContainer::get()['repository']->get(Issue::class)->updateById(
                            $issueId,
                            array(Field::FIELD_RESOLUTION_CODE => $updateValue),
                            $currentDate
                        );
                        $oldResolution = UbirimiContainer::get()['repository']->get(IssueSettings::class)->getById(
                            $issueData[Field::FIELD_RESOLUTION_CODE],
                            'resolution',
                            'name'
                        );
                        if ($updateValue) {
                            $newResolution = UbirimiContainer::get()['repository']->get(IssueSettings::class)->getById(
                                $updateValue,
                                'resolution',
                                'name'
                            );
            ...
            case WorkflowFunction::FUNCTION_SET_ISSUE_FIELD_VALUE:
                ...
                if ($field_value == -1) {
                    ...
                    switch ($field_name) {
                        ...
                        case Field::FIELD_RESOLUTION_CODE:
                            $fieldValueData = UbirimiContainer::get()['repository']->get(IssueSettings::class)->getById(
                                $field_value,
                                'resolution'
                            );
                            break;
