...
<div class="headerPageBackground">
    <table width="100%">
        <tr>
            <td>
                <div class="headerPageText"><a class="linkNoUnderline" href="/svn-hosting/repositories">Repositories</a>
                    > <a class="linkNoUnderline"
                         href="/svn-hosting/repository/<?php echo $svnRepo['id'] ?>"><?php echo $svnRepo['name'] ?></a>
                    > <?php echo $userData['first_name'] . ' ' . $userData['last_name'] ?> > Settings
                </div>
            </td>
        </tr>
    </table>
...
<div class="pageContent">
    ...
    <?php if (empty($userData['password'])): ?>
        <div class="messageGreen">You did not set a password for the SVN repository yet. Once you do, you can do the
            checkout.
        </div>
    <?php endif ?>
    ...
    <div>
        <table>
            <tr>
                <td>Password set:</td>
                <td><?php if (empty($userData['password'])) {
                        echo 'No';
                    } else echo 'Yes' ?></td>
            </tr>
            <tr>
                <td>Read Permission:</td>
                <td><?php if ($userData['has_read']) {
                        echo 'Yes';
                    } else echo 'No' ?></td>
            </tr>
            <tr>
                <td>Write Permission:</td>
                <td><?php if ($userData['has_write']) {
                        echo 'Yes';
                    } else echo 'No' ?></td>
            </tr>
        </table>
