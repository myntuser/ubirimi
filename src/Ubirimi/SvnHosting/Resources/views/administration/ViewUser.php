...
<div class="headerPageBackground">
    <table width="100%">
        <tr>
            <td>
                <div class="headerPageText">Administration > <a class="linkNoUnderline"
                                                                href="/svn-hosting/administration/all-repositories">Repositories</a>
                    > <?php echo $svnRepo['name'] ?> > Users
                </div>
            </td>
        </tr>
    </table>
    ...
    <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
        <tr>
            <?php if ($isSVNAdministrator): ?>
                <td>
                    <a id="btnNew" href="/general-settings/users/add?fsvn=<?php echo $request->get('id') ?>"
                       class="btn ubirimi-btn"><i class="icon-plus"></i> Create New User</a>
                </td>
            <?php endif ?>
            <?php if ($isSVNAdministrator): ?>
                <td>
            ...
            <td>
                <a id="btnDeleteSvnUser" href="#" class="btn ubirimi-btn disabled"><i class="icon-remove"></i> Delete
                    User</a>
            </td>
        </tr>
    </table>
        ...
        <table class="table table-hover table-condensed">
            ...
            <?php foreach ($svnRepoUserList as $repoUser): ?>
                <tr id="table_row_<?php echo $repoUser['user_id'] ?>">
                    ...
                    <td>
                        <input
                            type="checkbox" <?php if (1 == $repoUser['has_read']): ?> checked="checked" <?php endif ?>
                            value="1" disabled="disabled"/>
                    </td>
                    <td>
                        <input
                            type="checkbox" <?php if (1 == $repoUser['has_write']): ?> checked="checked" <?php endif ?>
                            value="1" disabled="disabled"/>
                    </td>
                    <td>
                        <input
                            type="checkbox" <?php if (!empty($repoUser['password'])): ?> checked="checked" <?php endif ?>
                            value="1" disabled="disabled"/>
                    </td>
                </tr>
            <?php endforeach ?>
