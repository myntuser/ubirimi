...
class SVNUtils
{
    ...
    public static function svnLookTransaction($command, $repository, $transaction)
    {
        ...
        return ConsoleUtils::runCmdCaptureMessage(
            SVNUtils::svnCommand("$command -t $transaction $repository"),
            $return
        );
    }
    ...
    {
        return SVNUtils::parseSvnVersion(
            ConsoleUtils::runCmdCaptureMessage(SVNUtils::svnCommand("--version --quiet"), $return)
        );
    }
    ...
    private static function _svnImport($server, $local)
    {
        ...
        $cmd = SVNUtils::svnCommand(
            "import --non-interactive --username Ubirimi -m \"" . "Commit by Ubirimi" . "\" $local $server"
        );
        $message = ConsoleUtils::runCmdCaptureMessage($cmd, $return);
        if ($return) {
            throw new USVN_Exception(
                "Can't import into subversion repository.\nCommand:\n" . $cmd . "\n\nError:\n" . $message
            );
        }
    }
