...
class UbirimiUser
{
    ...
    public function createAdministratorUser(
        $admin_first_name,
        $admin_last_name,
        $admin_username,
        $password,
    ...
    public function getUserAvatarPicture($user, $size = null)
    {
        if (null !== $user['avatar_picture'] && !empty($user['avatar_picture'])) {
            ...
            if ($size == 'small') {
                return '/assets' . UbirimiContainer::get(
                )['asset.user_avatar'] . $user['id'] . '/' . $fileName . '_33.' . $extension;
            } else {
                if ($size == 'big') {
                    return '/assets' . UbirimiContainer::get(
                    )['asset.user_avatar'] . $user['id'] . '/' . $fileName . '_150.' . $extension;
                }
            }
        }
