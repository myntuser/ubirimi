        ...
        <tr>
            <td>
                <table border="0" cellpadding="0" cellspacing="0" class="tableButtons">
                    <tr>
                        <td>
                            <input type="text" id="agile_quick_search" value="<?php echo $searchQuery ?>"
                                   class="inputText"
                                   style="width: 150px; background: url(/img/find.png) right no-repeat;"/>
                        </td>
                        <td>Quick Filters</td>
                        <td>
                            <?php
                            ...
                            ?>
                            <a id="btnAgileOnlyMyIssues"
                               href="/agile/board/plan/<?php echo $boardId ?><?php if (count($parametersLink)) {
                                   echo '?' . implode('&', $parametersLink);
                               } ?>" class="btn ubirimi-btn"><?php if ($onlyMyIssuesFlag)
                                    echo '<i class="icon-ok"></i>' ?> Only My Issues</a>
                        </td>
                        <td>
                            <a id="btnAddSprint" href="#" class="btn ubirimi-btn"><i class="icon-plus"></i> Create
                                Sprint</a>
                        </td>

                        <td>
                            <div class="btn-group" style="float: right;">
                                <a id="btnAddToSprint" href="#" class="btn ubirimi-btn dropdown-toggle disabled"
                                   data-toggle="dropdown">Add to Sprint <span class="caret"></span> </a>
                                ...
                                <?php if ($sprintsNotStarted || $currentStartedSprint): ?>
                                    ...
                                    <ul class="dropdown-menu">
                                        <?php if ($currentStartedSprint): ?>
                                            <li><a id="add_to_sprint_<?php echo $currentStartedSprint['id']; ?>"
                                                   href="#"><?php echo $currentStartedSprint['name'] ?></a></li>
                                        <?php endif ?>
                                        <?php while ($sprintsNotStarted && $sprint = $sprintsNotStarted->fetch_array(
                                                MYSQLI_ASSOC
                                            )): ?>
                                            <?php if (!$sprint['started_flag']): ?>
                                                <li><a id="add_to_sprint_<?php echo $sprint['id']; ?>"
                                                       href="#"><?php echo $sprint['name'] ?></a></li>
                                            <?php endif ?>
                                        <?php endwhile ?>
                                    </ul>
                                <?php endif ?>
            ...
            <td align="right">
                <ul class="nav nav-tabs" style="padding: 0px; float: right;">
                    ...
                    <li><a href="/agile/board/work/<?php if ($currentStartedSprint) {
                            echo $currentStartedSprint['id'] . '/' . $boardId;
                        } else {
                            echo '-1/' . $boardId;
                        } ?>" title="Work">Work</a></li>
                    <li><a href="/agile/board/report/<?php if ($lastCompletedSprint) {
                            echo $lastCompletedSprint['id'];
                        } else {
                            echo "-1";
                        } ?>/<?php echo $boardId ?>/sprint_report" title="Report">Report</a></li>
                </ul>
            </td>
        </tr>
    </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="">
        <tr>
            <td width="70%" valign="top" style="padding-right: 10px">
                <div id="agile_wrapper_planning" class="pageContentNoPadding"
                     style="height: 500px; overflow-y: auto; border-radius: 0px">
                    <?php if ($currentStartedSprint): ?>
                        <div style="padding-bottom: 4px;" class="droppablePlanSprint">
                            <span class="headerPageText"><?php echo $currentStartedSprint['name'] ?></span>
                            <?php
                            $issues = UbirimiContainer::get()['repository']->get(Sprint::class)->getIssuesBySprintId(
                                $currentStartedSprint['id'],
                                $onlyMyIssuesFlag,
                                $loggedInUserId,
                                $searchQuery
                            );
                            ...
                            $params = array(
                                ...
                                'checkbox_disabled' => $renderCheckboxDisabled,
                                'show_header' => false
                            );
                    ...
                    while ($sprintsNotStarted && $sprint = $sprintsNotStarted->fetch_array(MYSQLI_ASSOC)) {
                        ...
                        if (!$currentStartedSprint && $sprint['started_flag'] == 0 && !$sprintStartedTextRendered) {
                            if ($sprintHasIssues) {
                                echo '<a href="#" id="start_sprint_' . $sprint['id'] . '">Start Sprint</a> | ';
                            } else {
                                echo '<span>Start Sprint</span> | ';
                            }
                            $sprintStartedTextRendered = true;
                    ...
                    $issues = UbirimiContainer::get()['repository']->get(Board::class)->getBacklogIssues(
                        $clientId,
                        $board,
                        $onlyMyIssuesFlag,
                        $loggedInUserId,
            ...
            <td width="30%" valign="top">
                <div id="agileIssueContent" class="pageContentNoPadding" style="border-radius: 0px; overflow: auto">
                    ...
                    <div>Plan mode is where you scrub your backlog: review, estimate, and prioritise your stories and
                        bugs, then create and plan sprints (iterations of work).
                    </div>
                </div>
            </td>
        </tr>
...
<input type="hidden" value="agile" id="contextList"/> <input type="hidden"
                                                             value="<?php if ($sprintsNotStarted || $currentStartedSprint) {
                                                                 echo '1';
                                                             } else {
                                                                 echo '0';
                                                             } ?>" id="add_to_sprint_possible"/> <input type="hidden"
                                                                                                        value="<?php echo $boardId ?>"
                                                                                                        id="board_id"/>
