...
<?php Util::renderBreadCrumb(
    '<a class="linkNoUnderline" href="/agile/boards">Agile Boards</a> > ' . $board['name'] . ' > Configure'
) ?>
<div class="pageContent">
    <form name="add_board" action="/agile/board/add" method="post">
        ...
        <ul class="nav nav-tabs" style="padding: 0px;">
            <li id="p_tab_issues_overview"><a href="/agile/configure-board/<?php echo $boardId ?>"
                                              title="Filter">Filter</a></li>
            <li id="p_tab_issues" class="active"><a href="/agile/edit-board-columns/<?php echo $boardId ?>"
                                                    title="Columns">Columns</a></li>
            <li><a href="/agile/board-swimlane/<?php echo $boardId ?>" title="Issues">Swimlanes</a></li>
        </ul>

        <div>Columns can be added, removed, reordered and renamed. Columns are based upon global statuses and can be
            ...
            <table width="100%" cellspacing="0" border="0" cellpadding="0" id="agile_columns">
                <tr id="sortable" id="agile_columns_id">
                    <td width="<?php echo $columnWidth . '%'; ?>" valign="top" class="notSortable">
                        <table width="100%" cellspacing="0" cellpadding="4px" style="border: 1px solid #DDDDDD;"
                               class="notSortable">
                            <tr>
                            ...
                            <tr>
                                <td id="column_0" class="droppable" valign="top" style="height: 200px;">
                                    <?php for ($j = 0; $j < count($unmappedStatuses); $j++): ?>
                                        <div id="status_<?php echo $unmappedStatuses[$j]['id'] ?>"
                                             class="statusAgile draggable">
                                            &nbsp;<?php echo $unmappedStatuses[$j]['name'] ?></div>
                                    <?php endfor ?>
                                </td>
                            </tr>
                        </table>
                    ...
                    <?php for ($i = 0; $i < count($columns); $i++): ?>
                        <td width="<?php echo $columnWidth . '%'; ?>" valign="top" class="sortable draggableColumn"
                            id="agile_move_column_<?php echo $columns[$i]['id'] ?>">
                                ...
                                <tr>
                                    <td align="left" valign="top" style="border: 1px solid #DDDDDD;">
                                        ...
                                        <div style="float: right; width: 16px;">
                                            <img title="Delete Column"
                                                 id="deleteAgileColumn_<?php echo $columns[$i]['id'] ?>"
                                                 src="/img/delete.png"/>
                                        </div>
                                        <div style="clear: both;"></div>
                                        <div
                                            style="font-size: 16px; text-align: center;"><?php echo $columns[$i]['name'] ?></div>
                                    </td>
                                </tr>

                                <?php $statuses = UbirimiContainer::get()['repository']->get(
                                    Board::class
                                )->getColumnStatuses($columns[$i]['id'], 'array') ?>
                                <tr>
                                    <td id="column_<?php echo $columns[$i]['id'] ?>" class="droppable" valign="top"
                                        style="height: 200px;">
                                        <?php if ($statuses): ?>
                                            <?php for ($j = 0; $j < count($statuses); $j++): ?>
                                                <div id="status_<?php echo $statuses[$j]['id'] ?>"
                                                     class="statusAgile draggable">
                                                    &nbsp;<?php echo $statuses[$j]['name'] ?></div>
                                            <?php endfor ?>
                                        <?php endif ?>
                                    </td>
                                </tr>
