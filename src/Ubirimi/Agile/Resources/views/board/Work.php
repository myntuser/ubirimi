...
<div class="pageContent">
    <ul class="nav nav-tabs" style="padding: 0px; float: right;">
        ...
        <li class="active"><a href="/agile/board/work/<?php echo $sprintId ?>/<?php echo $boardId ?>"
        ...
        <tr>
            ...
            <td>
                <a id="btnAgileOnlyMyIssues"
                   href="/agile/board/work/<?php echo $sprintId ?>/<?php echo $boardId ?><?php if ($onlyMyIssuesFlag == 0)
                       echo '?only_my=1' ?>"
                   class="btn ubirimi-btn"><?php if ($onlyMyIssuesFlag) echo '<i class="icon-ok-sign"></i> ' ?>Only My
                    Issues</a>
            </td>
            <?php if ($sprint): ?>
                <td>
            ...
            <td valign="top" width="70%">
                <div id="agile_wrapper_work" style="height: 500px; overflow-y: auto;">
                    ...
                    if ($swimlaneStrategy == 'story' && $sprint) {
                        ...
                        if ($swimlaneStrategy == 'assignee' && $sprint) {
                            require_once __DIR__ . '/work/_swimlaneAssignee.php';
                        } else {
                            if ($swimlaneStrategy == 'no_swimlane' && $sprint) {
...
<?php
$lastColumnStatuses = UbirimiContainer::get()['repository']->get(Board::class)->getColumnStatuses(
    $columns[count($columns) - 1]['id'],
    'array',
    'id'
);
?>
...
<input type="hidden" value="<?php echo $boardId ?>" id="board_id"/> <input type="hidden" value="<?php echo $sprintId ?>"
                                                                           id="sprint_id"/> <input type="hidden"
                                                                                                   value="<?php echo count(
                                                                                                       $columns
                                                                                                   ); ?>"
                                                                                                   id="count_columns"/>
<input type="hidden" value="<?php echo $index; ?>" id="max_index_section"/> <input type="hidden"
                                                                                   value="<?php echo $swimlaneStrategy; ?>"
                                                                                   id="agile_swimlane_strategy"/>
