        ...
        <tr>
            <td valign="top" width="200px">
                <table width="100%" cellspacing="0" cellpadding="0">
                    <tr>
                        <td colspan="2" id="sectDates" class="sectionDetail"><span class="sectionDetailTitle">My Calendars</span>
                        </td>
                    </tr>
                    <?php if ($calendars): ?>
                        <?php foreach ($calendars as $calendar): ?>
                            <tr>
                                <td width="24px" valign="middle">
                                    <input <?php if (in_array(
                                        $calendar['id'],
                                        $calendarIds
                                    )) echo 'checked="checked"' ?> type="checkbox" value="<?php echo $calendar['id'] ?>"
                                                                   id="select_calendar_<?php echo $calendar['id'] ?>"/>
                                </td>
                ...
                <table width="100%" cellspacing="0" cellpadding="0">
                    <tr>
                        <td colspan="2" id="sectDates" class="sectionDetail"><span class="sectionDetailTitle">Shared with me</span>
                        </td>
                    </tr>
            ...
            <td>
                <table cellpadding="0" cellspacing="0" class="calendar">
                    <tr>
                        <td class="calendar-day-head"><?php echo implode(
                    ...
                    <?php for ($weekNumber = 1; $weekNumber <= $weeks; $weekNumber++): ?>
                        <tr class="calendar-row">
                            <?php for ($dayInWeek = 1; $dayInWeek <= 7; $dayInWeek++): ?>
                                ...
                                <td class="calendar-day<?php echo ' ' . $classCurrentDate ?>"
                                    id="calendar_day_<?php echo $dates[($weekNumber - 1) * 7 + $dayInWeek - 1] ?>"
                                    valign="top">
                                    ...
                                    <?php if ($calendarEvents): ?>
                                        ...
                                        <?php foreach ($calendarEvents as $event): ?>
                                            <?php if (substr($event['date_from'], 0, 10) <= $date && substr(
                                                    $event['date_to'],
                                                    0,
                                                    10
                                                ) >= $date
                                        ...
                                        while ($index <= $maxLevel) {
                                            ...
                                            foreach ($eventsToDisplay as $key => $event) {
                                                if ($eventData[$event['id']][0] == $index) {
                                                    ...
                                                    echo '<div>' . date(
                                                            "l, F Y",
                                                            strtotime($event['date_from'])
                                                        ) . ' - ' . date(
                                                            'l, F Y',
                                                            strtotime($event['date_to'])
                                                        ) . '</div>';
                                            ...
                                            if ($eventsPrinted > 5) {
                                                if ((count($eventsToDisplay) - $eventsPrinted) > 0) {
                                                    echo '<div class="show-all-events"><a id="show_all_events_' . $date . '" href="#">+' . (count(
                                                                $eventsToDisplay
                                                            ) - $eventsPrinted) . ' more</a></div>';
                                                }
