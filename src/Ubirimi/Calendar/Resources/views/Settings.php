...
<?php Util::renderBreadCrumb(
    '<a class="linkNoUnderline" href="/calendar/calendars">Calendars</a> > ' . $calendar['name'] . ' > Settings'
) ?>
...
<div class="pageContent">
    ...
    <form name="edit_status" action="/calendar/settings/<?php echo $calendarId ?>" method="post">
                ...
                <?php while ($defaultReminders && $defaultReminder = $defaultReminders->fetch_array(MYSQLI_ASSOC)): ?>
                    <div id="reminder_content_<?php echo $defaultReminder['id'] ?>">
                        ...
                        <select name="reminder_period_<?php echo $defaultReminder['id'] ?>" class="select2InputSmall">
                            <option <?php
                            if ($defaultReminder['cal_event_reminder_period_id'] == ReminderPeriod::PERIOD_MINUTE) echo 'selected="selected"' ?>
                                value="<?php echo ReminderPeriod::PERIOD_MINUTE ?>">minutes
                            </option>
                            <option <?php if ($defaultReminder['cal_event_reminder_period_id'] == ReminderPeriod::PERIOD_HOUR) echo 'selected="selected"' ?>
                                value="<?php echo ReminderPeriod::PERIOD_HOUR ?>">hours
                            </option>
                            <option <?php
                            if ($defaultReminder['cal_event_reminder_period_id'] == ReminderPeriod::PERIOD_DAY) echo 'selected="selected"' ?>
                                value="<?php echo ReminderPeriod::PERIOD_DAY ?>">days
                            </option>
                            <option <?php if ($defaultReminder['cal_event_reminder_period_id'] == ReminderPeriod::PERIOD_WEEK) echo 'selected="selected"' ?>
                                value="<?php
                                echo ReminderPeriod::PERIOD_WEEK ?>">weeks
                            </option>
                        </select>

                        <img src="/img/delete.png" id="delete_calendar_reminder_<?php echo $defaultReminder['id'] ?>"
        ...
        <br/>
        <button type="submit" name="edit_calendar_settings" class="btn ubirimi-btn"><i class="icon-edit"></i> Update
            Settings
        </button>
