...
<div class="pageContent">
    <?php if (Util::userHasClientAdministrationPermission()): ?>
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <?php if ($smtpSettings): ?>
                    <td><a href="#" class="btn ubirimi-btn disabled"><i class="icon-plus"></i> Configure New SMTP Server</a>
                    </td>
                    <td>
                        <?php if ($smtpSettings['default_ubirimi_server_flag']): ?>
                            <a class="btn ubirimi-btn disabled"><i class="icon-edit"></i> Edit</a>
                        <?php else: ?>
                            <a href="/general-settings/smtp-settings/edit/<?php echo $smtpSettings['id'] ?>"
                               class="btn ubirimi-btn"><i class="icon-edit"></i> Edit</a>
                        <?php endif ?>
                    </td>
                    <td><a id="btnDeleteSMTPServer" href="#" class="btn ubirimi-btn"><i class="icon-remove"></i> Delete</a>
                    </td>
                <?php else: ?>
                    <td><a href="/general-settings/smtp-settings/add" class="btn ubirimi-btn"><i class="icon-plus"></i>
                            Configure New SMTP Server</a></td>
                    <?php if (!$smtpSettings['default_ubirimi_server_flag']): ?>
