...
<div class="pageContent">
    <?php if (Util::userHasClientAdministrationPermission()): ?>
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <td><a href="/general-settings/users/add" class="btn ubirimi-btn"><i class="icon-plus"></i> Create User</a>
                </td>
                <td><a id="btnEditUser" href="#" class="btn ubirimi-btn"><i class="icon-edit"></i> Edit</a></td>
                <td><a id="btnDeleteUser" href="#" class="btn ubirimi-btn disabled"><i class="icon-remove"></i>
                        Delete</a></td>
            </tr>
        </table>
    <?php endif ?>
    <table class="table table-hover table-condensed">
        ...
        <?php while ($user = $users->fetch_array(MYSQLI_ASSOC)): ?>
            <tr <?php if (Util::userHasClientAdministrationPermission()
            ): ?> id="table_row_<?php echo $user['id'] ?>" <?php endif ?>>
                <?php if (Util::userHasClientAdministrationPermission()): ?>
                    <td width="22">
