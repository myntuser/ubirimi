...
if ($page) {
    ...
    if ($parentPage) {
        $breadCrumb .= LinkHelper::getDocumentadorPageLink(
                $parentPage['id'],
                $parentPage['name'],
                'linkNoUnderline'
            ) . ' > ';
...
<div class="pageContent" style="overflow: hidden; margin-left: 285px">
    <?php if ($page): ?>
        ...
        if ($lastRevision) {
            ...
            $lastEditedText .= LinkHelper::getUserProfileLink(
                    $lastRevision['user_id'],
                    SystemProduct::SYS_PRODUCT_DOCUMENTADOR,
                    $lastRevision['first_name'],
                    $lastRevision['last_name']
                ) . ' on ' . $date;
        } else {
        ...
        ?>
        <div class="smallDescription"><?php echo $linkAttachments ?> Added by <?php echo LinkHelper::getUserProfileLink(
                $page['user_id'],
                SystemProduct::SYS_PRODUCT_DOCUMENTADOR,
                $page['first_name'],
                $page['last_name']
            ) ?>, <?php echo $lastEditedText ?></div>
        ...
        <?php if ($page['documentator_entity_type_id'] == EntityType::ENTITY_FILE_LIST): ?>
            ...
            <?php endif ?>
            <br/>
            <form name="page_upload_file" method="post" enctype="multipart/form-data"
                  action="/documentador/entity/upload/<?php echo $entityId ?>">
                <div style="border: dashed blue 1px; padding: 8px">
                    ...
                    <input style="padding: 4px" name="entity_upload_file[]" type="file" multiple=""
                           value="Upload Files"/>
        ...
        <div>
            ...
            <div id="pageCommentsSection" style="display: block; clear: both;">
                ...
                <?php if ($comments): ?>
                    <div class="headerPageText" style="border-bottom: 1px solid #DDDDDD;"><?php echo count($comments) ?>
                        Comment<?php if (count($comments) > 1) echo 's' ?></div>
                    <div style="float: left; display: block; width: 100%">
                        <?php
                        $htmlLayout = '';
                        UbirimiContainer::get()['repository']->get(EntityComment::class)->getCommentsLayoutHTML(
                            $comments,
                            $htmlLayout,
                            null,
                            0
                ...
                <?php if (Util::checkUserIsLoggedIn()): ?>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                        <tr>
                            <td>
                                <div>
                                    <input type="button" name="add_comment" id="doc_view_page_add_comment"
                                           value="Add Comment" class="btn ubirimi-btn"/>
                                </div>
                            </td>
                        </tr>
                    </table>
