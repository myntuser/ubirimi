...
<div class="pageContent">
    ...
    <ul class="nav nav-tabs" style="padding: 0px;">
        <li><?php echo LinkHelper::getUserProfileLink(
                $userId,
                ...
                <tr>
                    <td>
                        <?php if ($activity['action'] == 'created'): ?>
                            <?php echo LinkHelper::getDocumentadorPageLink(
                                    $activity['id'],
                                    $activity['name']
                                ) . ' created on ' . Util::getFormattedDate(
                                    $activity['date'],
                                    $clientSettings['timezone']
                                ) ?>
                        <?php elseif ($activity['action'] == 'created_space'): ?>
                            <a href="/documentador/pages/<?php echo $activity['id'] ?>"><?php echo $activity['name'] ?></a> created on <?php echo Util::getFormattedDate(
                                $activity['date'],
                                $clientSettings['timezone']
                            ) ?>
                        <?php elseif ($activity['action'] == 'edited'): ?>
                            <?php echo LinkHelper::getDocumentadorPageLink(
                                    $activity['id'],
                                    $activity['name']
                                ) . ' updated on ' . Util::getFormattedDate(
                                    $activity['date'],
                                    $clientSettings['timezone']
                                ) ?>
                        <?php elseif ($activity['action'] == 'comment'): ?>
                            <?php echo LinkHelper::getDocumentadorPageLink(
                                    $activity['id'],
                                    $activity['name']
                                ) . ' commented on ' . Util::getFormattedDate(
                                    $activity['date'],
                                    $clientSettings['timezone']
                                ) ?>
                        <?php endif ?>
