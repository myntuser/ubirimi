...
<div class="pageContent">
    <form name="edit_space_permissions"
          action="/documentador/administration/space-tools/edit-permissions/<?php echo $spaceId ?>" method="post">
        ...
        <?php if ($groups): ?>
            <table class="table table-hover table-condensed">
                ...
                <?php while ($group = $groups->fetch_array(MYSQLI_ASSOC)): ?>
                    <?php $groupPermission = UbirimiContainer::get()['repository']->get(
                        Space::class
                    )->getGroupPermission($spaceId, $group['id']) ?>
                    <tr>
                        <td><?php echo $group['name'] ?></td>
                        <td align="center">
                            <?php if ($groupPermission && $groupPermission['all_view_flag']): ?>
                                <input type="checkbox" value="1" checked="checked"
                                       name="space_group_all_view_flag_<?php echo $group['id'] ?>"/>
                            <?php else: ?>
                        ...
                        <td align="center">
                            <?php if ($groupPermission && $groupPermission['space_admin_flag']): ?>
                                <input type="checkbox" value="1" checked="checked"
                                       name="space_group_space_admin_flag_<?php echo $group['id'] ?>"/>
                            <?php else: ?>
                                <input type="checkbox" value="1"
                                       name="space_group_space_admin_flag_<?php echo $group['id'] ?>"/>
                            <?php endif ?>
        ...
        <table class="table table-hover table-condensed">
            ...
            <?php while ($user = $users->fetch_array(MYSQLI_ASSOC)): ?>
                <?php $userPermission = UbirimiContainer::get()['repository']->get(Space::class)->getUserPermission(
                    $spaceId,
                    $user['id']
                ) ?>
                <tr>
                    ...
                    <td align="center">
                        <?php if ($userPermission && $userPermission['all_view_flag']): ?>
                            <input type="checkbox" value="1" checked="checked"
                                   name="space_user_all_view_flag_<?php echo $user['id'] ?>"/>
                        <?php else: ?>
                    ...
                    <td align="center">
                        <?php if ($userPermission && $userPermission['space_admin_flag']): ?>
                            <input type="checkbox" value="1" checked="checked"
                                   name="space_user_space_admin_flag_<?php echo $user['id'] ?>"/>
                        <?php else: ?>
                            <input type="checkbox" value="1"
                                   name="space_user_space_admin_flag_<?php echo $user['id'] ?>"/>
                        <?php endif ?>
        ...
        <table class="table table-hover table-condensed">
            ...
            <tr>
                ...
                <td align="center">
                    <input type="checkbox" value="1"
                           name="anonymous_all_view_flag" <?php if ($anonymousAccessSettings['all_view_flag']) {
        ...
        <table width="100%" id="table_list" cellpadding="0" cellspacing="0">
            <tr>
                <td align="left" colspan="2">
                    <button type="submit" name="update_configuration" class="btn ubirimi-btn"><i class="icon-edit"></i>
                        Update
                    </button>
                    <a class="btn ubirimi-btn"
                       href="/documentador/administration/space-tools/permissions/<?php echo $spaceId ?>">Cancel</a>
                </td>
            </tr>
        </table>
    </form>
