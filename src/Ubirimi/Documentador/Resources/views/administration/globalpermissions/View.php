...
<div class="pageContent">
    <?php if (Util::userHasDocumentadorAdministrativePermission()): ?>
        <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
            <tr>
                <td><a href="/documentador/administration/edit-global-permissions" class="btn ubirimi-btn"><i
                            class="icon-edit"></i> Edit Permissions</a></td>
            </tr>
        </table>
        ...
        <div>For example: Enabling anonymous 'USE' permission, allows non-logged-in users to browse pages and spaces in
            Documentador.
        </div>
        <table class="table table-hover table-condensed">
            ...
            <tr>
                ...
                <td align="center"><?php if ($documentatorSettings['anonymous_use_flag']) {
                        echo 'YES';
                    } else echo 'NO' ?></td>
                <td align="center"><?php if ($documentatorSettings['anonymous_view_user_profile_flag']) {
                        echo 'YES';
                    } else echo 'NO' ?></td>
            </tr>
            </tbody>
                ...
                <?php while ($group = $groups->fetch_array(MYSQLI_ASSOC)): ?>
                    <tr>
                        ...
                        <?php while ($globalsPermission = $globalsPermissions->fetch_array(MYSQLI_ASSOC)): ?>
                            <td align="center">
                                <?php
                                $data = UbirimiContainer::get()['repository']->get(
                                    GlobalPermission::class
                                )->getDataByPermissionIdAndGroupId($clientId, $globalsPermission['id'], $group['id']);
                                if ($data) {
                                    echo 'YES';
                                } else {
                                    echo 'NO';
                ...
                <tr>
                    ...
                    <?php while ($globalsPermission = $globalsPermissions->fetch_array(MYSQLI_ASSOC)): ?>
                        <td align="center">
                            ...
                            $data = UbirimiContainer::get()['repository']->get(
                                GlobalPermission::class
                            )->getDataByPermissionIdAndUserId($clientId, $globalsPermission['id'], $user['id']);
                            if ($data) {
                                echo 'YES';
                            } else {
                                echo 'NO';
