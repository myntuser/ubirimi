    ...
    <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
        <tr>
            <?php if (isset($organization)): ?>
                <td><a href="/helpdesk/administration/customers/add?id=<?php echo $organization['id'] ?>"
                       class="btn ubirimi-btn">Add Customer</a></td>
            <?php else: ?>
                <td><a href="/helpdesk/administration/customers/add" class="btn ubirimi-btn">Add Customer</a></td>
            <?php endif ?>
            <td><a id="btnEditCustomer" href="#" class="btn ubirimi-btn disabled"><i class="icon-edit"></i> Edit</a>
            </td>
            <td><a id="btnDeleteCustomer" href="#" class="btn ubirimi-btn disabled"><i class="icon-remove"></i>
                    Delete</a></td>
        </tr>
    </table>
        ...
        <table class="table table-hover table-condensed">
            ...
            <?php while ($customer = $customers->fetch_array(MYSQLI_ASSOC)): ?>
                <tr id="table_row_<?php echo $customer['id'] ?>">
                    ...
                    <td><?php if (isset($organization)) {
                            echo $organization['name'];
                        } else echo $customer['organization_name'] ?></td>
                </tr>
            <?php endwhile ?>
            </tbody>
        </table>
