...
<div class="pageContent">
    ...
    <table cellspacing="0" border="0" cellpadding="0" class="tableButtons">
        <tr>
            <td>
                <a href="/helpdesk/sla/calendar/add/<?php echo $projectId ?>" class="btn ubirimi-btn"><i
                        class="icon-plus"></i> Create Calendar</a>
                <a href="#" class="btn ubirimi-btn disabled" id="btnEditSLACalendar"><i class="icon-edit"></i> Edit</a>
                <a href="#" id="btnDeleteSLACalendar" class="btn ubirimi-btn disabled"><i class="icon-remove"></i>
                    Delete</a>
            </td>
        </tr>
    </table>
    ...
    <?php if ($calendars): ?>
        <table class="table table-hover table-condensed">
            ...
            <?php while ($calendar = $calendars->fetch_array(MYSQLI_ASSOC)): ?>
                <?php $slasCalendar = UbirimiContainer::get()['repository']->get(Sla::class)->getByCalendarId(
                    $clientId,
                    $calendar['id']
                ) ?>
                <input type="hidden" id="delete_possible_<?php echo $calendar['id'] ?>"
                       value="<?php if ($slasCalendar) {
                           echo '0';
                       } else echo '1' ?>"/>
                <tr id="table_row_<?php echo $calendar['id'] ?>">
                    ...
                    <td>
                        ...
                        $data = UbirimiContainer::get()['repository']->get(
                            SlaCalendar::class
                        )->getCalendarDataByCalendarId($calendar['id']);
                        ...
                        <table style="width: 350px">
                            ...
                            <?php for ($i = 0; $i < count($data); $i++): ?>
                                ...
                                <tr>
                                    ...
                                    <td><?php if ($data[$i]['not_working_flag']) {
                                            echo 'No';
                                        } else {
                                            echo 'Yes';
                                        } ?></td>
